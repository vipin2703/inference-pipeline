...
LANGFUSE TRACING:
  - `langfuse.openai` ka AsyncOpenAI drop-in wrapper use kar rahe hain --
    ...
    Langfuse me trace ho jaata hai, koi extra manual logging code nahi likhna
    padta.
  - @observe() decorator function-level trace banata hai (taaki agar kal
    is function ke andar aur bhi steps add ho -- jaise retrieval, reranking --
    to sab ek hi trace ke andar nested dikhein).
...
#   LANGFUSE_PUBLIC_KEY, LANGFUSE_SECRET_KEY, LANGFUSE_HOST (ya LANGFUSE_BASE_URL)
# Ye backend/vllm/.env me set hone chahiye (Langfuse project ki API keys).
